...
for(const file of ['allocation.ts','planting.ts','cross-section.ts','roundabout.ts','slip-model.ts','model.ts','geometry.ts','slip-geometry.ts','design-validation.ts','lane-configuration.ts','scene-surfaces.ts','arrow-layout.ts','objects.ts','furniture3d.ts']){
...
const windowProfile=sp.windowStationProfile(100,20,80,15,10,1),windowStations=sp.profileSampleStations(windowProfile);assert.equal(sp.valueAtStation(windowProfile,10),0);assert.equal(sp.valueAtStation(windowProfile,20),0);assert.equal(sp.valueAtStation(windowProfile,35),1);assert.equal(sp.valueAtStation(windowProfile,70),1);assert.equal(sp.valueAtStation(windowProfile,80),0);assert(windowStations.includes(20)&&windowStations.includes(35)&&windowStations.includes(70)&&windowStations.includes(80),'window profile must expose lifecycle break stations');
...
const legacyV2=JSON.parse(JSON.stringify(removed));legacyV2.schemaVersion=2;legacyV2.links.forEach(link=>delete link.components);const migratedV2=n.normalizeNetworkProject(legacyV2);assert.equal(migratedV2.schemaVersion,3,'Network v2 must migrate to v3');assert(migratedV2.links.every(link=>Array.isArray(link.components)&&link.components.length===0),'v2 migration must introduce an empty persisted station-component list');
const legacyV1=JSON.parse(JSON.stringify(removed));legacyV1.schemaVersion=1;legacyV1.links.forEach(link=>{delete link.sectionProfile;delete link.components;link.via=link.via.map(v=>({x:v.x,y:v.y}));});const migratedV1=n.normalizeNetworkProject(legacyV1);assert.equal(migratedV1.schemaVersion,3,'Network v1 must migrate directly to v3');assert(migratedV1.links.every(link=>link.sectionProfile.mode==='review'));assert(migratedV1.links.every(link=>link.components.length===0));assert(migratedV1.links.every(link=>link.via.every(v=>v.radius===0)),'legacy via points must preserve their old sharp geometry');
...
const reviewGeometry=lg.resolveLinkSectionGeometry(transitionProject,transitionLink),reviewFrom=n.linkEndSection(transitionProject,transitionLink,'from'),reviewTo=n.linkEndSection(transitionProject,transitionLink,'to');assert(Math.abs(reviewGeometry.left[0]-(reviewFrom.median/2+reviewFrom.forwardLanes*reviewFrom.forwardLaneWidth))<1e-8,'Review collar must match FROM Arm section exactly at its port');assert(Math.abs(reviewGeometry.left.at(-1)-(reviewTo.median/2+reviewTo.forwardLanes*reviewTo.forwardLaneWidth))<1e-8,'Review collar must match TO Arm section exactly at its port');
...
const stationBaseGeometry=lg.resolveLinkSectionGeometry(transitioned,transitionedLink),stationBasePoints=n.linkPoints(transitioned,transitionedLink),stationTotal=stationBaseGeometry.total,
  laneStart=stationTotal*.2,laneEnd=stationTotal*.8,laneTaper=Math.min(20,stationTotal*.15),laneMid=(laneStart+laneEnd)/2;
const laneAdded=n.addLinkStationComponent(transitioned,transitionedLink.id,'lane');assert.equal(laneAdded.error,null);const laneComponent=laneAdded.project.links[0].components[0];assert.equal(laneComponent.kind,'lane');const laneConfigured=n.updateLinkStationComponent(laneAdded.project,transitionedLink.id,laneComponent.id,{start:laneStart,end:laneEnd,taperIn:laneTaper,taperOut:laneTaper,side:'curb'});assert.equal(laneConfigured.error,null);const laneLink=laneConfigured.project.links[0],laneGeometry=lg.resolveLinkSectionGeometry(laneConfigured.project,laneLink),baseLeftMid=sp.sampleStationSeries(stationBaseGeometry.stations,stationBaseGeometry.left,laneMid),laneLeftMid=sp.sampleStationSeries(laneGeometry.stations,laneGeometry.left,laneMid),laneWidthMid=sp.sampleStationSeries(laneGeometry.stations,laneGeometry.forwardLaneWidth,laneMid);assert(Math.abs((laneLeftMid-baseLeftMid)-laneWidthMid)<1e-6,'persisted auxiliary lane must add exactly one local lane width at full activation');assert(laneGeometry.stations.some(value=>Math.abs(value-laneStart)<.02)&&laneGeometry.stations.some(value=>Math.abs(value-laneEnd)<.02),'station lifecycle must insert its persisted boundary stations into resolved geometry');assert(laneGeometry.points.length>=stationBasePoints.length,'station lifecycle densification must never reduce the resolved alignment samples');assert(laneGeometry.laneLines.some(line=>line.id==='forward:component:'+laneComponent.id&&line.offsets.some(v=>v===null)&&line.offsets.some(v=>v!==null)),'auxiliary lane divider must exist only inside its persisted station lifecycle');
const overlappingLane=n.addLinkStationComponent(laneConfigured.project,transitionedLink.id,'lane');assert(overlappingLane.error&&/ห้ามซ้อนช่วง station/.test(overlappingLane.error),'same-direction auxiliary lane lifecycles must not overlap ambiguously');
const widthAdded=n.addLinkStationComponent(laneConfigured.project,transitionedLink.id,'width');assert.equal(widthAdded.error,null);const widthComponent=widthAdded.project.links[0].components.find(component=>component.kind==='width'),widthStart=stationTotal*.25,widthEnd=stationTotal*.75,widthTaper=Math.min(15,stationTotal*.12),walkMid=(widthStart+widthEnd)/2;const widthConfigured=n.updateLinkStationComponent(widthAdded.project,transitionedLink.id,widthComponent.id,{direction:'forward',target:'walk',delta:1.25,start:widthStart,end:widthEnd,taperIn:widthTaper,taperOut:widthTaper});assert.equal(widthConfigured.error,null);const widthLink=widthConfigured.project.links[0],widthGeometry=lg.resolveLinkSectionGeometry(widthConfigured.project,widthLink),widthWalk=sp.sampleStationSeries(widthGeometry.stations,widthGeometry.forwardWalk.outer,walkMid)-sp.sampleStationSeries(widthGeometry.stations,widthGeometry.forwardWalk.inner,walkMid),baseWalk=sp.sampleStationSeries(stationBaseGeometry.stations,stationBaseGeometry.forwardWalk.outer,walkMid)-sp.sampleStationSeries(stationBaseGeometry.stations,stationBaseGeometry.forwardWalk.inner,walkMid);assert(Math.abs((widthWalk-baseWalk)-1.25)<1e-6,'edge-width lifecycle must apply its persisted delta to the shared sidewalk profile');assert.equal(n.updateLinkSectionProfile(widthConfigured.project,transitionedLink.id,'review'),widthConfigured.project,'Review mode must not silently discard persisted station components');
const componentSaved=JSON.stringify(widthConfigured.project),componentRestored=n.restoreNetworkProject(componentSaved);assert.deepEqual(componentRestored,widthConfigured.project,'Network v3 station components must round-trip through project JSON');
const componentScene=sg.resolveRoadLinkSceneSurfaces(widthConfigured.project);assert(componentScene.some(surface=>surface.id===transitionedLink.id+':road')&&componentScene.some(surface=>surface.id===transitionedLink.id+':forward-walk'),'Network 3D must consume the same resolved station-component geometry as 2D');
...
const localJunctionSurfaces=js.resolveJunctionSceneSurfaces(removed.junctions[0].design);assert(localJunctionSurfaces.some(v=>v.id==='main-pavement'&&v.kind==='road'),'Junction 3D resolver must consume the authoritative base Junction footprint');assert(localJunctionSurfaces.some(v=>v.kind==='sidewalk'&&v.z>.1),'Junction 3D resolver must expose raised sidewalks');assert(localJunctionSurfaces.some(v=>v.kind==='median'&&v.z>.1),'Junction 3D resolver must expose median/island surfaces');
const slipDesign=structuredClone(removed.junctions[0].design);slipDesign.slips=[{id:'slip-0',fromArm:0,toArm:1,width:4,radius:32,approach:{mode:'direct'},departure:{mode:'direct'},crossing:{enabled:false,offset:10}}];const slipScene=js.resolveJunctionSceneSurfaces(slipDesign);assert(slipScene.some(v=>v.id==='slip:slip-0:pavement'),'Slip v6 pavement must enter Network 3D through slip-geometry ownership');assert(slipScene.some(v=>v.id==='slip:slip-0:island'),'Slip v6 island must enter Network 3D as a raised semantic surface');
const worldProject=structuredClone(removed),worldJunction=worldProject.junctions[0];worldJunction.x+=17;worldJunction.y-=11;worldJunction.rotation=37;const localRoad=js.resolveJunctionSceneSurfaces(worldJunction.design).find(v=>v.id==='main-pavement'),worldRoad=sg.resolveJunctionSceneSurfaces(worldProject).find(v=>v.id===worldJunction.id+':main-pavement');assert(localRoad&&worldRoad);const angle=n.worldJunctionRotation(worldJunction)*Math.PI/180,lp=localRoad.points[0],expected={x:worldJunction.x+lp.x*Math.cos(angle)-lp.y*Math.sin(angle),y:worldJunction.y+lp.x*Math.sin(angle)+lp.y*Math.cos(angle)};assert(Math.hypot(worldRoad.points[0].x-expected.x,worldRoad.points[0].y-expected.y)<1e-7,'Network 3D must transform Junction local geometry through JunctionInstance world transform only');
const fullScene=sg.resolveNetworkSceneSurfaces(transitioned);assert(fullScene.some(v=>v.id.startsWith(transitioned.junctions[0].id+':'))&&fullScene.some(v=>v.id.startsWith(transitioned.links[0].id+':')),'resolved Network 3D scene must contain both Junction and RoadLink semantic surfaces');
const furnitureProject=structuredClone(removed);furnitureProject.junctions[0].design.arms[0].signal=true;const sceneFaces=sg.resolveJunctionSceneFaces(furnitureProject);assert(sceneFaces.length>0&&sceneFaces.every(face=>face.points.every(p=>Number.isFinite(p.x)&&Number.isFinite(p.y)&&Number.isFinite(p.z))),'Network 3D must transform shared Junction furniture into finite world-space faces');
...
assert.equal(n.restoreNetworkProject('{bad').schemaVersion,3);
...
console.log('PASS network project v3: persisted station components, shared station profiles, PI radius curves, explicit curb/median lane transitions, local-image A-B calibration, v1/v2 migration, direct Arm edits, rigid Map Align transform, semantic ports, Complete Streets continuity and persistence');
