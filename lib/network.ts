...
import {corridorWarning} from '../app/junction/cross-section';
...
export function pocketArm(r:Road,c:Component):Arm{const a=initial().arms[0];return{...a,median:r.config.median,width:c.width,corridorMode:'preserve',incomingPockets:{...emptyPockets(),right:{lanes:1,length:c.storage,taper:c.taper}}};}
export function pocketIssue(r:Road,c:Component){const warning=corridorWarning(pocketArm(r,c));if(warning)return warning;const ps=vertices(r),total=lengthOf(ps),s=c.t*total,frame=station(ps,s),before=lengthOf(ps.slice(0,frame.index+1)),after=before+Math.hypot(ps[frame.index+1].x-ps[frame.index].x,ps[frame.index+1].y-ps[frame.index].y),available=r.config.traffic==='Left'?s-before:after-s;return available<c.storage+c.taper+2?'Geometry Error — ช่วงเลนและสอบต้องอยู่บนท่อนตรงเดียว เพิ่มความยาวท่อนหรือย้ายตำแหน่ง':null;}
