...
import {armMouth,treatmentOrigins,medianEdges} from './geometry';
...
export function plantingPlan(d:Design,id:number){const a=d.arms[id],s={...medianTreeDefaults(),...a.medianTrees},mouth=armMouth(d,id),origins=treatmentOrigins(a,mouth,d.type==='roundabout'),margin=s.crown/2+.2,requested=mouth+s.start,zones:Interval[]=[];
 ...
 const allocation=allocate(a,0,origins);for(const dir of ['incoming','outgoing'] as const){const p=allocation.features[dir].right;if(p.medianUsed>0)zones.push({start:mouth,end:p.end+margin,reason:'เลนชิดเกาะและช่วงสอบ'});}
 ...
 let narrow:number|null=null;for(let x=mouth;x<=a.length+.25;x+=.25){const [lo,hi]=medianEdges(a,x,origins),bad=hi-lo<s.crown+.4;if(bad&&narrow===null)narrow=x;if(!bad&&narrow!==null){zones.push({start:narrow-margin,end:x+margin,reason:'เกาะแคบกว่าพื้นที่พุ่มที่กำหนด'});narrow=null;}}if(narrow!==null)zones.push({start:narrow-margin,end:a.length,reason:'เกาะแคบกว่าพื้นที่พุ่มที่กำหนด'});
 const merged=mergeZones(zones),intervals=validIntervals(Math.max(mouth,requested),a.length-margin,merged),trees:{x:number;y:number;height:number;size:number}[]=[];if(s.enabled&&a.median>0)for(const range of intervals){const first=Math.max(range.start,(trees.at(-1)?.x??-Infinity)+s.spacing);for(let x=first;x<=range.end+1e-8;x+=s.spacing){const [lo,hi]=medianEdges(a,x,origins),y=Math.max(lo+margin,Math.min(hi-margin,(lo+hi)/2+s.offset));trees.push({x,y,height:s.height,size:s.crown});}}
