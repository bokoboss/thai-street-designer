...
function CameraNum({label,value,min,max,step=1,onCommit}:{label:string;value:number;min:number;max:number;step?:number;onCommit:(v:number)=>void}){
...
export default function Scene3D({d,active,ref,onSave,onNotice,mapReference,mapCredentials}:{d:Design;active:boolean;ref?:React.Ref<SceneHandle>;onSave:(format:'png'|'jpeg')=>void;onNotice:(message:string)=>void;mapReference:MapReference;mapCredentials:MapProviderCredentials}){
 ...
 const faces=useMemo(()=>{const fs:Face[]=[];
 for(const surface of resolveJunctionSceneSurfaces(d)){
  if(surface.kind==='sidewalk')fs.push(...prism(surface.points,surface.z,'#bdc9ce','#8d9ca4'));
  else if(surface.kind==='median')fs.push(...prism(surface.points,surface.z,'#86a58a','#9caa9d'));
 }
 ...
 const objectAngle=(yaw+d.rotation)*Math.PI/180,mapAngle=yaw*Math.PI/180,p=pitch*Math.PI/180,scale=Math.min(w,h)/(extent*2)*zoom;
 const projectAt=(v:V,a:number)=>{const x=v.x*Math.cos(a)-v.y*Math.sin(a),y=v.x*Math.sin(a)+v.y*Math.cos(a);return {x:w/2+pan.x*w+x*scale,y:h/2+pan.y*h+(y*Math.cos(p)-v.z*Math.sin(p))*scale,depth:y*Math.sin(p)+v.z*Math.cos(p)};};
 const project=(v:V)=>projectAt(v,objectAngle);
 if(mapImg&&mapReference.enabled){const mapExtent=extent*Math.SQRT2,origin=projectAt(vertex(-mapExtent,-mapExtent,0),mapAngle),x=projectAt(vertex(mapExtent,-mapExtent,0),mapAngle),y=projectAt(vertex(-mapExtent,mapExtent,0),mapAngle);ctx.save();ctx.globalAlpha=Math.max(.1,Math.min(1,mapReference.opacity));ctx.transform((x.x-origin.x)/mapImg.width,(x.y-origin.y)/mapImg.width,(y.x-origin.x)/mapImg.height,(y.y-origin.y)/mapImg.height,origin.x,origin.y);ctx.drawImage(mapImg,0,0);ctx.restore();}
 ...
 useEffect(()=>{if(!active)return;const c=canvas.current,ctx=c?.getContext('2d');if(!c||!ctx)return;const ratio=Math.min(window.devicePixelRatio||1,2);c.width=size.w*ratio;c.height=size.h*ratio;ctx.setTransform(ratio,0,0,ratio,0,0);draw(ctx,size.w,size.h,'#e9eff2',texture,activeMapTexture);},[active,size,texture,activeMapTexture,faces,yaw,pitch,zoom,pan,extent,d.rotation,mapReference.enabled,mapReference.opacity]);
 ...
 function resetCamera(){gestures.current.clear();drag.current=null;applyCamera({yaw:-30,pitch:52,zoom:1,pan:{x:0,y:0}});}
 function saveReferenceCamera(){try{localStorage.setItem('thai-street-reference-view-3d',JSON.stringify(camera.current));onNotice('บันทึกมุมมองอ้างอิง 3D แล้ว');}catch{onNotice('บันทึกมุมมองอ้างอิง 3D ไม่ได้');}}
 function restoreReferenceCamera(){try{const v=JSON.parse(localStorage.getItem('thai-street-reference-view-3d')||'null');if(!v||!Number.isFinite(v.yaw)||!Number.isFinite(v.pitch)||!Number.isFinite(v.zoom)||!Number.isFinite(v.pan?.x)||!Number.isFinite(v.pan?.y))throw Error();applyCamera({yaw:v.yaw,pitch:Math.max(5,Math.min(85,v.pitch)),zoom:clampZoom(v.zoom),pan:v.pan});onNotice('ใช้มุมมองอ้างอิง 3D แล้ว');}catch{onNotice('ยังไม่มีมุมมองอ้างอิง 3D ที่ใช้ได้');}}
 ...
 if(!active)return null;return <div className="j-scene"><ContextMenu onOpenChange={open=>{if(open){gestures.current.clear();drag.current=null;}}}><ContextMenuTrigger asChild><canvas ref={canvas} tabIndex={0} aria-label="แบบสามมิติ คลิกซ้ายเลื่อน คลิกกลางหมุน ล้อเมาส์ซูม คลิกขวาเมนูภาพ" onPointerDown={begin} onPointerMove={move} onPointerUp={end} onPointerCancel={end} onLostPointerCapture={end} onAuxClick={e=>e.preventDefault()} onWheel={wheel}/></ContextMenuTrigger><ContextMenuContent><ContextMenuItem onSelect={()=>onSave('png')}>บันทึกภาพ PNG · โปร่งใส</ContextMenuItem><ContextMenuItem onSelect={()=>onSave('jpeg')}>บันทึกภาพ JPEG · พื้นขาว</ContextMenuItem><ContextMenuItem onSelect={copyImage}>คัดลอกภาพ 3D</ContextMenuItem><ContextMenuSeparator/><ContextMenuItem onSelect={resetCamera}>คืนมุมมองเริ่มต้น</ContextMenuItem></ContextMenuContent></ContextMenu><div className="j-scene-note">{mapReference.enabled&&mapLoading?'กำลังเตรียมแผนที่อ้างอิงบนพื้น 3D…':mapReference.enabled?'แผนที่อ้างอิงแสดงบนพื้น 3D · ไม่ติดไปกับ export':''}{mapReference.enabled&&<br/>}เมาส์: ซ้ายเลื่อน · กลางหมุน · ล้อซูม · ขวาเมนูภาพ<br/>สัมผัส: นิ้วเดียว{mode==='rotate'?'หมุน':'เลื่อน'} · สองนิ้วเลื่อนและบีบซูม</div><div className="j-scene-tools"><button aria-pressed={mode==='rotate'} onClick={()=>setMode('rotate')}>หมุน</button><button aria-pressed={mode==='pan'} onClick={()=>setMode('pan')}>เลื่อน</button><CameraNum label="Yaw °" value={+yaw.toFixed(1)} min={-360} max={360} step={1} onCommit={v=>applyCamera({...camera.current,yaw:v})}/><CameraNum label="Pitch °" value={+pitch.toFixed(1)} min={5} max={85} step={1} onCommit={v=>applyCamera({...camera.current,pitch:v})}/><CameraNum label="Zoom %" value={Math.round(zoom*100)} min={35} max={500} step={5} onCommit={v=>applyCamera({...camera.current,zoom:v/100})}/><button onClick={resetCamera}>ค่าเริ่มต้น</button><button onClick={saveReferenceCamera}>จำมุม</button><button onClick={restoreReferenceCamera}>ใช้มุม</button></div></div>;
}
