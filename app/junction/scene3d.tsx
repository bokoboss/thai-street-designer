...
export default function Scene3D({d,active,ref,onSave,onNotice}:{d:Design;active:boolean;ref?:React.Ref<SceneHandle>;onSave:(format:'png'|'jpeg')=>void;onNotice:(message:string)=>void}){
 ...
 const faces=useMemo(()=>{const fs:Face[]=[],edgeSet=edges(d),slips=slipGeometries(d,edgeSet);for(const e of edgeSet){const ps=[...e.outer,...e.walk.slice().reverse()].map(p=>rotate(p,armTurn(d,e.i)));fs.push(...prism(ps,.18,'#bdc9ce','#8d9ca4'));}
 for(const g of slips){for(const island of [g.island,g.raisedSeparator])if(island.length)fs.push(...prism(island.map(p=>rotate(p,armTurn(d,g.fromArm))),.18,'#c7cdbd','#9caa9d'));}
