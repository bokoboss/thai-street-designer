...
import {activeIds,bounds,carBounds,armMouth,stopPosition,rotate,armTurn} from './geometry';
...
export function furnitureFaces(d:Design):Face[]{const fs:Face[]=[];
 ...
 for(const i of activeIds(d)){const core=armMouth(d,i),a=d.arms[i];if(!a.signal||!a.incoming)continue;const x=d.type==='roundabout'?core+5:stopPosition(a,core),post=bounds(a)[1]+Math.min(.5,sectionFor(a,'incoming').walk/2),center=(a.median/2+carBounds(a)[1])/2,h=6;
