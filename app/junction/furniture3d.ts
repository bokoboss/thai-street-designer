...
import {activeIds,innerEdge,bounds,carBounds,armMouth,armTreatmentOrigins,edges,rotate} from './geometry';
...
export function signalFrame(d:Design,i:number,edgeSet=edges(d)){
 const a=d.arms[i],core=armMouth(d,i),origins=armTreatmentOrigins(d,i,edgeSet),x=d.type==='roundabout'?core+5:origins.incoming,
 post=bounds(a,x,origins)[1]+Math.min(.5,sectionFor(a,'incoming').walk/2),
 center=(innerEdge(a,1,x,origins)+carBounds(a,x,origins)[1])/2;
 return{x,post,center,origins};
}
export function furnitureFaces(d:Design):Face[]{const fs:Face[]=[],edgeSet=edges(d);
 ...
 for(const i of activeIds(d)){const a=d.arms[i];if(!a.signal||!a.incoming)continue;const {x,post,center}=signalFrame(d,i,edgeSet),h=6;
