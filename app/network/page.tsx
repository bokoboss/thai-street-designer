...
import './style.css';
import MapBackground,{BASEMAP_OPTIONS,MAP_REFERENCE_STORAGE,mapReferenceDefaults,restoreMapReference,searchMapPlaces,type MapBasemap,type MapPlace,type MapReference} from '../junction/map-background';
...
import {
  NETWORK_EDIT_JUNCTION_STORAGE,NETWORK_PROJECT_STORAGE,addJunction,connectPorts,createNetworkProject,insertLinkVia,junctionById,linkControlPoints,linkIssues,linkLength,linkLinearTransitionPossible,moveJunction,moveLinkVia,portKey,
  projectBounds,removeJunction,removeLink,removeLinkVia,restoreNetworkProject,rotateJunction,setJunctionArmEnabled,updateJunctionArmBasics,updateJunctionArmGeometry,updateJunctionArmPocket,updateJunctionArmSection,updateLinkSectionProfile,updateLinkViaRadius,type NetworkProject,type PortRef,type WorldPoint
    ...
    <div className="network-body">
      ...
      <aside className="network-inspector">
        ...
        {selectedLink&&<section>
          ...
          <div className="network-inline-actions"><button onClick={()=>{const ps=linkControlPoints(projectRef.current,selectedLink);let best=0,bestLen=-1;for(let i=0;i<ps.length-1;i++){const len=Math.hypot(ps[i+1].x-ps[i].x,ps[i+1].y-ps[i].y);if(len>bestLen){best=i;bestLen=len;}}const p={x:(ps[best].x+ps[best+1].x)/2,y:(ps[best].y+ps[i=best+1].y)/2};const before=projectRef.current,next=insertLinkVia(before,selectedLink.id,best,p);if(next!==before){commit(next,before);setSelectedLinkVertex(best);setNotice('เพิ่ม PI แล้ว · ค่าเริ่มต้น R25 m และลากจุดเพื่อปรับ alignment ได้');}}}>＋ PI / จุดแนว</button><button disabled={selectedLinkVertex===null} onClick={()=>{if(selectedLinkVertex===null)return;const before=projectRef.current,next=removeLinkVia(before,selectedLink.id,selectedLinkVertex);if(next!==before){commit(next,before);setSelectedLinkVertex(null);}}}>ลบ PI</button></div>
          ...
          <div className="network-link-profile"><div className="network-arm-editor-head"><span>SECTION CONTINUITY</span><b>{selectedLink.sectionProfile.mode==='linear'?'Linear transition':'Review mismatch'}</b></div><label>การต่อหน้าตัด<select value={selectedLink.sectionProfile.mode} onChange={e=>{const before=projectRef.current,next=updateLinkSectionProfile(before,selectedLink.id,e.target.value as 'review'|'linear');if(next===before&&e.target.value==='linear'){setNotice('ใช้ Linear transition ไม่ได้เมื่อจำนวนเลนหรือชนิด edge bands สองปลายไม่ตรงกัน');return;}commit(next,before);setNotice(e.target.value==='linear'?'เปิด geometric section transition แล้ว':'กลับเป็นโหมดตรวจ mismatch แล้ว');}}><option value="review">Review mismatch</option><option value="linear" disabled={!linearTransitionPossible}>Linear geometric transition</option></select></label><p className="network-note">Linear transition แก้ความกว้างเลน เกาะกลาง sidewalk และ band widths แบบต่อเนื่องเมื่อ topology สองปลายเหมือนกัน หากจำนวนเลนหรือชนิด band ต่างกัน ระบบยังบังคับให้แก้ semantic transition แยกต่างหาก</p></div>
